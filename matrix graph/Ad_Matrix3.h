...
Graph* createGraph(int number){
    ...
    graph->weight=(int**)malloc(sizeof(int*)*number);
    for(i=0;i<number;i++){
        graph->Node[i]=(int*)malloc(sizeof(int)*number);
        graph->weight[i]=(int*)malloc(sizeof(int)*number);
    }
    return graph;
}
...
void newEdge(Graph* graph,int startNode, int endNode,int w){
    if(graph->vertex<(startNode-1)||graph->vertex<(endNode)-1) {
        printf("그래프 정점 번호 오류\n");
        return;
    }
    graph->Node[startNode][endNode]=1;
    graph->Node[endNode][startNode]=1;
    graph->weight[startNode][endNode]=w;
    graph->weight[endNode][startNode]=w;
